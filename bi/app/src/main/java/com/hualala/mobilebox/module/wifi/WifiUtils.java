...
public class WifiUtils {
    ...
    private static void removeWifiBySsid(WifiManager wifiManager, String targetSsid) {
        ...
        if (configs != null) {
            for (WifiConfiguration config : configs) {
                ...
                printLog("delete_config" + targetSsid);
                //}
            }
        }
    ...
    public static List<WifiBean> getWifiList(Context context) {
        List<WifiBean> wifis = new ArrayList<>();
        List<ScanResult> scanResults = noSameName(getWifiScanResult(context));
        WifiBean wifi = null;
        if (scanResults != null && !scanResults.isEmpty()) {
            for (int i = 0; i < scanResults.size(); i++) {
                wifi = new WifiBean();
                wifi.setWifiName(scanResults.get(i).SSID);
                wifi.setCapabilities(scanResults.get(i).capabilities);
                wifi.setLevel(getLevel(scanResults.get(i).level) + "");
                wifi.setScanResult(scanResults.get(i));
                wifis.add(wifi);
